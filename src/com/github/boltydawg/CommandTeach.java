...
public class CommandTeach implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		else if(Main.scoreboard.getObjective("class").getScore(player.getName()).getScore()!=2) {sender.sendMessage("Only Mages can learn spells!"); return true;}
		else if(!Spells.isValid(args[1])) {sender.sendMessage("Invalid spell"); return true;}
		else if(Main.mages.get(player.getUniqueId()).contains(args[1])) {player.sendMessage(ChatColor.GRAY+"You already know this spell"); return true;}
		else {
			if(Main.mages.get(player.getUniqueId()).size()>=7) {
				...
				Main.mages.get(player.getUniqueId()).add(args[1]);
