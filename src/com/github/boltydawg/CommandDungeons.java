	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args[0].equalsIgnoreCase("start")) {
			...
			for(int i=0; i<Bukkit.getOfflinePlayers().length; i++) {
				Main.scoreboard.getObjective("pillars").getScore(Bukkit.getOfflinePlayers()[i].getName()).setScore(0);
				Main.scoreboard.getObjective("zombie").getScore(Bukkit.getOfflinePlayers()[i].getName()).setScore(0);
				Main.scoreboard.getObjective("water").getScore(Bukkit.getOfflinePlayers()[i].getName()).setScore(0);
			}
	...
	public boolean startDungeon(ArrayList<Player> players, String dungeon) {
		if(dungeon.equalsIgnoreCase("pillars")) {
			if(pillars.size()>0) {
				...
				for(Player player : players){
					if(Main.scoreboard.getObjective("pillars").getScore(player.getName()).getScore()>0) {
				...
				if(players.size()<2) {
					...
					for(Player player : players) {
						...
						Main.scoreboard.getObjective("pillars").getScore(player.getName()).setScore(1);
			...
			if(zombie.size()>0) {
				...
				for(Player player : players){
					if(Main.scoreboard.getObjective("zombie").getScore(player.getName()).getScore()>0) {
						player.sendMessage(ChatColor.YELLOW+"You've already completed this dungeon!");
						players.remove(player);
					}
				}
				if(players.size()<2) {
					...
					for(Player player : players) {
						player.setGameMode(GameMode.ADVENTURE);
						Main.scoreboard.getObjective("zombie").getScore(player.getName()).setScore(1);
						zombie.add(player.getUniqueId());
						
						if(Main.scoreboard.getObjective("class").getScore(player.getName()).getScore()==2) {
							spells.put(player.getUniqueId(), Main.mages.get(player.getUniqueId()));
							Main.mages.get(player.getUniqueId()).clear();
						}
			...
			if(water.size()>0) {
				...
				for(Player player : players){
					if(Main.scoreboard.getObjective("water").getScore(player.getName()).getScore()>0) {
						player.sendMessage(ChatColor.YELLOW+"You've already completed this dungeon!");
						players.remove(player);
					}
				}
				if(players.size()<2) {
					...
					for(Player player : players) {
						player.setGameMode(GameMode.ADVENTURE);
						Main.scoreboard.getObjective("water").getScore(player.getName()).setScore(1);
	...
	public boolean endDungeon(String dungeon) {
		if(dungeon.equalsIgnoreCase("pillars")) {
			for(UUID u : pillars) {
				...
				Main.scoreboard.getObjective("pillars").getScore(player.getName()).setScore(2);
				pillars.remove(u);
			}
			...
			for(UUID u : zombie) {
				Player player = Bukkit.getPlayer(u);
				Main.scoreboard.getObjective("zombie").getScore(player.getName()).setScore(2);
				
				if(Main.scoreboard.getObjective("class").getScore(player.getName()).getScore()==2) {
					Main.mages.put(u, spells.get(u));
					spells.remove(u);
				}
			...
			for(UUID u : water) {
				Player player = Bukkit.getPlayer(u);
				Main.scoreboard.getObjective("water").getScore(player.getName()).setScore(2);
				water.remove(u);
			}
